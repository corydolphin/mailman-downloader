...
class MyArgumentParser(ArgumentParser):
    def __init__(self):
        ...
        self.add_argument("list_archive_urls", nargs='+', help="Root url(s) for archive")
        ...
        self.add_argument("--force", help="Flag to overwrites files stored locally", action = 'store_const', const=True)
...
def downloadList(mailingListUrl, username=None, password=None, dest=None, force=True):
    ...
    for url in getArchiveUrls(br, mailingListUrl):
        ...
        try:
            overWrote, cached = downloadAndDecodeArchive(br, url, rootDir=rootDir, overWrite=force)
            if cached:
...
def download(list_archive_urls, username=None, password=None, dest=None, force=True):
    '''
    The main entry point for this module. This function will download all archives of all
    mailing lists specified in the list_archive_urls list, with username and password
    used for authenticating with Mailman.
    ...
    for list_archive_url in list_archive_urls:
        try:
            downloadList(list_archive_url, username=username, password=password, dest=dest, force=force)
        except Exception, e:
             print >>sys.stderr, "An error occured while downloading '%s'" % getListNameFromUrl(list_archive_url)
             traceback.print_exc()
...
def main():
    ...
    try:
        ...
        download(args.list_archive_urls, username=args.username, password=args.password, dest=args.dest, force=args.force)
